...
while running:
    ...
    for _, origen, destino, arc_id in caneria_origen_destino:
        ...
        text = font.render(
            str(f"{arc_id},y:{round(flujo_caneria_particular,1)}"), True, BLACK)
    ...
    for i, pos in enumerate(positions):
        ...
        for fila in demandas_ldl:
            if fila[:3] == [comuna_actual, i, t_actual]:
                valor_demanda = fila[3] if i in nodos_demanda else 0
        ##
    ...
    t_text = font.render(
        f"t = {t_actual}, comuna = {comuna_actual}, plata = {plata_ldl[t_actual][1]}", True, BLACK)
