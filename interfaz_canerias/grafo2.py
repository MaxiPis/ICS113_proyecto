...
while running:
    ...
    for _, origen, destino, arc_id in caneria_origen_destino:
        ...
        if mostrar_linea:
            color = colorear_binarias(entrada, booleano) if entrada in variables_binarias else GRAY
            grosor = max(2, min(6, int(flujo * 0.8)))
            pygame.draw.line(screen, color, positions[origen], positions[destino], grosor)
    ...
    for i, pos in enumerate(positions):
        ...
        if valor_demanda > 0:
            ...
            demanda_rect = demanda_text.get_rect(midtop=(pos[0], pos[1] + node_radius + 5))
