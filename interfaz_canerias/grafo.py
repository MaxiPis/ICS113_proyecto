...
csv_s = parent_path.joinpath("dataset")
resultados = parent_path.joinpath("resultados")
...
#print("ssssssssssss",df_canerias.iloc[0].to_list())
#print("largo", len(df_canerias))
...
# Obtener valores de tiempo y comunas##################################################################
tiempos_disponibles = sorted(estado_por_t_bin.keys())
comunas_disponibles = [i for i in range(cantidad_de_comunas)]
...
while running:
    ...
    for _ , origen, destino, arc_id in caneria_origen_destino:
        ...
        ####COLOREADO##############################################################################
        color = GRAY
        if entrada in variables_binarias:
            booleano = estado_actual_binarias.get(key, 0.0) == 1.0
            color = colorear_binarias(entrada, booleano)    
        # Y 
        estado_variable_y_values = estado_variable_y.get(t_actual,{})
        flujo_caneria_particular = estado_variable_y_values.get(key_for_y, 0.0)
        
        # X
        #estado_variable_x_values = estado_variable_x.get(t_actual,{})
        #print( "values de X(f,n,s,t)", estado_variable_x_values)
        ...
        text = font.render(str(f"{arc_id},y:{round(flujo_caneria_particular,1)}"), True, BLACK)
    ...
    for i, pos in enumerate(positions):
        ...
        for fila in demandas_ldl:
            if fila[:3] == [comuna_actual, i, t_actual]:
                valor_demanda = fila[3] if i in terreno_demanda else 0   
        ##

        num_text = font.render(str(f"{i}, dem:{valor_demanda}"), True, BLACK)
        screen.blit(num_text, (pos[0] - 7, pos[1] - 7))
    #########################################################################################################

    # Mostrar datos actuales #################################################################################
    t_text = font.render(f"t = {t_actual}, comuna = {comuna_actual}, plata = {plata_ldl[t_actual][1]}", True, BLACK)
    ...
    for event in pygame.event.get():
        ...
        # Imprimir estado de 'x' al presionar tecla X sin afectar visualización
            elif event.key == pygame.K_x:
                estado_x_t = estado_variable_x.get(t_actual, {})
                #estado_variable_x_values = estado_variable_x.get(t_actual,{})
        #print( "values de X(f,n,s,t)", estado_variable_x_values)
                print(f"--- Estado de 'x' en t={t_actual}, comuna={comuna_actual} ---")
